...
// let setCheck,
//     myRadios = document.getElementsByName("tabs2");
// for (let t = 0; t < myRadios.length; t++)
//     myRadios[t].onclick = (t) => {
...
//             if (e) {
//                 (total.count = e.value),
//                     1 == document.getElementById("custom-input-number").value
//                         ? ((btnIncr = document.getElementById("decrement")), btnIncr.setAttribute("disabled", !0))
//                         : btnIncr.removeAttribute("disabled");
//             }
//             let n = total.count,
//                 a = moment(total.date_first, "DD.MM.YYYY"),
//                 s = moment(total.date_last, "DD.MM.YYYY").add(1, "days").diff(a, "days");
//             if (document.getElementById("daysOf")) {
//                 let e = s;
//                 (e %= 10),
//                     (document.getElementById("daysOf").innerText =
//                         1 == e ? "день" : 2 == e || 3 == e || 4 == e ? "дня" : "дней");
//                 let a = s;
//                 (a %= 10),
//                     1 == a
//                         ? (document.getElementById("daysOf").innerText = "день")
//                         : (document.getElementById("daysOf1").innerText = 2 == a || 3 == a || 4 == a ? "дня" : "дней");
//                 let i = 0;
//                 for (let t in total.rezhim) i = parseInt(total.rezhim[t]);
//                 (ColHours = i),
//                     (totalCost = (getTarif + t) * i * n * s),
//                     (totalCostWo = parseFloat(totalCost / 1.2).toFixed(0));
//                 let o = totalCostWo;
//                 (o %= 10),
//                     (ndsCost = parseFloat(getTarif + t - parseFloat((getTarif + t) / 1.2).toFixed(2)).toFixed(2));
//                 let l = ndsCost;
//                 (l %= 10),
//                     0 == l &&
//                         (ndsCost = parseFloat(getTarif + t - parseFloat((getTarif + t) / 1.2).toFixed(2)).toFixed(0));
//                 let r = 10 * Math.ceil(parseFloat((getTarif + t) / 1.2).toFixed(0) / 10);
//                 (totalCostWo = r * i * n * s),
//                     (document.getElementById("tarif").innerHTML = Intl.NumberFormat("ru-RU").format(getTarif + t)),
//                     (document.getElementById("tarifWo").innerHTML = Intl.NumberFormat("ru-RU").format(r)),
//                     (document.getElementById("nds").innerHTML = ndsCost),
//                     (document.getElementById("hours").innerHTML = (ColHours / 10) * s),
//                     (document.getElementById("hoursS").innerHTML = ColHours * s),
//                     (document.getElementById("hours_1").innerHTML = ColHours * s),
//                     (document.getElementById("hours1").innerHTML = (ColHours / 10) * s),
//                     (document.getElementById("days").innerHTML = s),
//                     (document.getElementById("days1").innerHTML = s),
//                     (document.getElementById("totalCost").innerHTML =
//                         Intl.NumberFormat("ru-RU").format(totalCost) + " руб"),
//                     (document.getElementById("totalCostWo").innerHTML =
//                         Intl.NumberFormat("ru-RU").format(totalCostWo) + " руб");
//             }
//         },
//     },
...
// }
// activeBtn &&
//     (activeBtn.classList.add("not-shadow-mass-button"), loadparams(activeBtn.getAttribute("data-id")).then(start));
...
//                         if (t[e].classList.contains("activated")) {
...
//                         if (e[i].classList.contains("activated")) {
...
//                 else {
//                     if (
//                         (t.parentNode.parentNode.parentNode.querySelectorAll(".selected-2 button").forEach((t) => {
//                             t.addEventListener("click", () => {
//                                 let e = t.parentNode.querySelectorAll("button");
//                                 for (let t = 0; t < e.length; t++)
//                                     e[t].classList.remove("activated", "not-shadow-mass-button");
//                                 t.classList.add("activated", "not-shadow-mass-button");
//                             });
//                         }),
//                         t.classList.toggle("activated"),
//                         t.classList.toggle("not-shadow-mass-button"),
...
//     dp2,
//     date_1,
//     m = { dateFirst: 0, dateLast: 0 };
// function fun1() {
//     let t = document.getElementById("myinput");
//     if (t) {
//         let n = document.querySelector(".bubble"),
//             a = t.min,
//             s = t.max,
//             i = t.value;
//         function e(t, e) {
...
//                 a = t.min ? t.min : 0,
//                 s = t.max ? t.max : 100,
//                 i = Number((100 * (n - a)) / (s - a));
//             (e.innerHTML = n), (e.style.left = `calc(${i}% + (${12 - 0.19 * i}px))`);
//         }
//         (t.style.background = `linear-gradient(to right, #999999 0%, #999999 ${((i - a) / (s - a)) * 100}%, #f5f5f5 ${
//             ((i - a) / (s - a)) * 100
//         }%, #f5f5f5 100%)`),
//             (t.oninput = function () {
//                 this.style.background = `linear-gradient(to right, #999999 0%, #999999 ${
//                     ((this.value - this.min) / (this.max - this.min)) * 100
//                 }%, #f5f5f5 ${((this.value - this.min) / (this.max - this.min)) * 100}%, #f5f5f5 100%)`;
//             }),
...
//     let t = 527,
//         e = 0,
//         n = document.getElementsByClassName("column");
//     for (let e = 0; e <= n.length - 1; e++) n[e].offsetHeight > t && (t = n[e].offsetHeight);
//     n.length > 0 && (e = n[0].offsetWidth);
//     for (let a = 0; a <= n.length - 1; a++)
//         (n[a].style.height = t + "px"),
//             a >= n.length - 2 && ((n[a].style.width = e + "px"), n[a].classList.remove("flex-grow"));
// }
...
//                 (date_1 = e),
//                 dp2.render(),
//                 dp2.getDate() <= e && (dp2.setDate(e), (total.date_last = date_1)),
...
//     btn = document.getElementsByClassName("myBtn-open");
// for (let t of btn)
//     t.onclick = () => {
...
//                       (dh = e.clientHeight),
//                       (dh = e.clientHeight || e.offsetHeight + 5),
...
//                     (t.style.paddingTop = "1.25rem"),
//                     (t.style.justifyContent = "flex-between");
//                 for (i = 0; i <= 105; i += 5)
//                     !(function () {
//                         var e = i;
//                         setTimeout(function () {
...
//                     var e = i;
//                     setTimeout(function () {
//                         (t.style.height = (e / 100) * a + "px"),
//                             e <= 0 && ((t.style.display = "none"), (t.style.height = o));
//                     }, 200 - 2 * e);
//                 })();
//             return !0;
//         });
...
//         let t = document.getElementById("success");
...
//             screen.width < 414
//                 ? t.setAttribute("content", "width=414, user-scalable=no")
//                 : t.setAttribute("content", "width=device-width, initial-scale=1.0, user-scalable=no");
//         }
//         t(),
...
//         initDate() {
//             let t;
//             (t = this.selectedDate ? new Date(Date.parse(this.selectedDate)) : new Date()),
//                 (this.month = t.getMonth()),
//                 (this.year = t.getFullYear()),
//                 (this.datepickerValue = this.formatDateForDisplay(t));
//         },
//         formatDateForDisplay(t) {
//             let e = DAYS[t.getDay()],
//                 n = ("0" + t.getDate()).slice(-2),
//                 a = MONTH_NAMES[t.getMonth()],
//                 s = MONTH_SHORT_NAMES[t.getMonth()],
//                 i = ("0" + (parseInt(t.getMonth()) + 1)).slice(-2),
//                 o = t.getFullYear();
//             return "DD-MM-YYYY" === this.dateFormat
//                 ? `${n}.${i}.${o}`
//                 : "YYYY-MM-DD" === this.dateFormat
//                 ? `${o}-${i}-${n}`
//                 : "D d M, Y" === this.dateFormat
//                 ? `${e} ${n} ${s} ${o}`
//                 : `${e} ${n} ${a} ${o}`;
//         },
//         isSelectedDate(t) {
...
//         isToday(t) {
//             const e = new Date(),
//                 n = new Date(this.year, this.month, t);
...
//     for (let t of e)
//         t.classList.contains("activated") &&
//             (t.classList.remove("activated"), t.classList.remove("not-shadow-mass-button"));
// }
// function push_btn(t) {
//     t.currentTarget.parentNode.parentElement.classList.contains("selected-1") &&
//         functionName(t.currentTarget.parentNode.parentElement),
